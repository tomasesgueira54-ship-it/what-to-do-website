...
import { toAbsolute, cleanText, safeDate, extractPriceFromHtml } from './scrapers/utils';
...
function extractDetailsFromHtml(html: string, eventUrl: string): {
    ...
    const description =
        normalizeSingleLine(primaryLd?.description) ||
        normalizeSingleLine($('meta[name="description"], meta[property="og:description"]').first().attr('content')) ||
        normalizeSingleLine(
...
export function extractDateFromHtml(html: string): string {
    try {
        ...
        const textDate = parsePtDate($.text());
...
function mergeEvents(target: Event, incoming: Event): void {
    ...
    if (!target.price && incoming.price) {
...
function dedupe(events: Event[]): Event[] {
    ...
    const getKeys = (ev: Event) => {
        ...
        return [normUrl, `${titleKey}|${day}|${ev.source}`, `${titleKey}|${day}|${ev.location.toLowerCase()}`];
    };
